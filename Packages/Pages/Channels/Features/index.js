...
function wireEvents() {
  ...
  document.getElementById('ch-connect-telegram')?.addEventListener('click', async () => {
    ...
    if (!token && !hasSaved) {
      setFb('telegram', 'Paste your bot token first.', 'error');
      return;
    }
  ...
  document.getElementById('ch-connect-whatsapp')?.addEventListener('click', async () => {
    ...
    if (!sid && !sidSaved) {
      setFb('whatsapp', 'Enter your Account SID.', 'error');
      return;
    }
    if (!token && !tokenSaved) {
      setFb('whatsapp', 'Enter your Auth Token.', 'error');
      return;
    }
    if (!number) {
      setFb('whatsapp', 'Enter the sandbox number (with whatsapp: prefix).', 'error');
      return;
    }
  ...
  document.getElementById('ch-connect-discord')?.addEventListener('click', async () => {
    ...
    if (!token && !tokenSaved) {
      setFb('discord', 'Enter your Bot Token.', 'error');
      return;
    }
    if (!channel) {
      setFb('discord', 'Enter the Channel ID.', 'error');
      return;
    }
  ...
  document.getElementById('ch-connect-slack')?.addEventListener('click', async () => {
    ...
    if (!token && !tokenSaved) {
      setFb('slack', 'Enter your Bot Token (xoxb-…).', 'error');
      return;
    }
    if (!channel) {
      setFb('slack', 'Enter the Channel ID (starts with C).', 'error');
      return;
    }
...
async function disconnectChannel(name) {
  ...
  try {
    ...
    if (name === 'telegram') {
      ...
      if (t) {
        t.value = '';
        t.placeholder = '1234567890:ABCdef\u2026';
      }
    } else if (name === 'whatsapp') {
      const s = document.getElementById('ch-wa-sid');
      if (s) {
        s.value = '';
        s.placeholder = 'ACxxxxxxxxxxxxxxxx';
      }
      const t = document.getElementById('ch-wa-token');
      if (t) {
        t.value = '';
        t.placeholder = 'Your auth token';
      }
