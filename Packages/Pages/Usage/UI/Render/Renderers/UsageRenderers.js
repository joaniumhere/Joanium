...
export function renderInsights(stats, records) {
  ...
  if (efficientModels.length > 0) {
    ...
    const costPer1k = (
      bestEfficient.cost /
      ((bestEfficient.input + bestEfficient.output) / 1_000)
    ).toFixed(6);
    insights.push({
      icon: 'Eff',
...
export function renderSummary(stats, records) {
  ...
  const cards = [
    ...
    },
    {
      ...
      sub:
        stats.count > 0
          ? `over ${stats.count} call${stats.count !== 1 ? 's' : ''}`
          : 'no calls yet',
      cls: '',
    },
  ];
  ...
  element.innerHTML = recent
    .map((record) => {
      ...
      const cost = (() => {
        ...
        return (
          ((record.inputTokens ?? 0) / 1_000_000) * p.in +
          ((record.outputTokens ?? 0) / 1_000_000) * p.out
        );
      })();
      return `
