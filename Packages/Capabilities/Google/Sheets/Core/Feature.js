    ...
    methods: {
      ...
      async readRange(ctx, { spreadsheetId, range }) {
        return withGoogle(ctx, async (credentials) => {
          if (!spreadsheetId || !range)
            return { ok: false, error: 'spreadsheetId and range are required' };
      ...
      async writeRange(ctx, { spreadsheetId, range, values }) {
        return withGoogle(ctx, async (credentials) => {
          if (!spreadsheetId || !range || !values)
            return { ok: false, error: 'spreadsheetId, range, and values are required' };
          return {
            ok: true,
      ...
      async clearRange(ctx, { spreadsheetId, range }) {
        return withGoogle(ctx, async (credentials) => {
          ...
          return {
            ok: true,
            result: await SheetsAPI.clearRange(credentials, spreadsheetId, range),
          };
        });
      },
      ...
      async addSheet(ctx, { spreadsheetId, title }) {
        return withGoogle(ctx, async (credentials) => {
          if (!spreadsheetId || !title)
            return { ok: false, error: 'spreadsheetId and title are required' };
