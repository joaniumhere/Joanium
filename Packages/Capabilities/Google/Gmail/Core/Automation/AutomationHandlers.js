...
export const gmailAutomationHandlers = {
  ...
  async gmail_reply(ctx, action) {
    ...
    await sendNotification('Reply sent', `Replied to message ${action.messageId}`);
  },
  ...
  async gmail_forward(ctx, action) {
    ...
    await sendNotification('Email forwarded', `Forwarded to ${action.forwardTo}`);
  },
  ...
  async gmail_create_draft(ctx, action) {
    ...
    await sendNotification('Draft saved', `To: ${action.to} · Subject: ${action.subject}`);
  },
  ...
  async gmail_label_emails(ctx, action) {
    ...
    if (!emails.length) {
      await sendNotification('No emails to label', `Query: "${action.query}"`);
      return;
    }
