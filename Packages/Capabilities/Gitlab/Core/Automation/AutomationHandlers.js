...
export const gitlabDataSourceCollectors = {
  async gitlab_notifications(ctx) {
    ...
    if (!notifications.length) return 'EMPTY: GitLab has no pending todos/notifications.';
    return `GitLab Notifications - ${notifications.length} pending:\n\n${notifications
      .slice(0, 15)
      .map(
        (item, index) =>
  ...
  async gitlab_repos(ctx, dataSource) {
    const credentials = requireGitlabCredentials(ctx);
    const repos = await GitlabAPI.getRepos(credentials, dataSource.maxResults ?? 30);
    if (!repos.length) return 'EMPTY: No GitLab repositories found.';
    return `GitLab Repositories - ${repos.length} repos:\n\n${repos
      .map((repo, index) => `${index + 1}. ${repo.full_name} [${repo.language ?? 'unknown'}]`)
      .join('\n')}`;
  },
  ...
  async gitlab_prs(ctx, dataSource) {
    ...
    const prs = await GitlabAPI.getPullRequests(
      credentials,
      dataSource.owner,
      dataSource.repo,
      state,
    ...
    if (!prs.length)
      return `EMPTY: ${dataSource.owner}/${dataSource.repo} has no ${state} merge requests.`;
    return `GitLab Merge Requests (${dataSource.owner}/${dataSource.repo}) - ${prs.length}:\n\n${prs
      .map(
        (pr, index) => `${index + 1}. !${pr.number}: ${pr.title} by ${pr.user?.login ?? 'unknown'}`,
      )
      .join('\n\n')}`;
  },
  ...
  async gitlab_issues(ctx, dataSource) {
    ...
    const credentials = requireGitlabCredentials(ctx);
    const state = dataSource.state ?? 'open';
    const issues = await GitlabAPI.getIssues(
      credentials,
      dataSource.owner,
      dataSource.repo,
      state,
    ...
    return `GitLab Issues (${dataSource.owner}/${dataSource.repo}) - ${issues.length}:\n\n${issues
      .map(
        (issue, index) =>
          `${index + 1}. #${issue.number}: ${issue.title} by ${issue.user?.login ?? 'unknown'}`,
      )
      .join('\n\n')}`;
  },
  ...
  async gitlab_commits(ctx, dataSource) {
    ...
    return `GitLab Commits (${dataSource.owner}/${dataSource.repo}) - ${commits.length}:\n\n${commits
      .map(
        (commit, index) =>
          `${index + 1}. ${String(commit.commit?.message || '').split('\n')[0]} - ${commit.commit?.author?.name ?? 'unknown'}`,
      )
      .join('\n')}`;
  },
  ...
  async gitlab_releases(ctx, dataSource) {
    requireRepo(dataSource.owner, dataSource.repo);
    const credentials = requireGitlabCredentials(ctx);
    const releases = await GitlabAPI.getReleases(
      credentials,
      dataSource.owner,
      dataSource.repo,
      dataSource.maxResults ?? 10,
    ...
    return `GitLab Releases (${dataSource.owner}/${dataSource.repo}) - ${releases.length}:\n\n${releases
      .map(
        (release, index) =>
  ...
  async gitlab_workflow_runs(ctx, dataSource) {
    requireRepo(dataSource.owner, dataSource.repo);
    const credentials = requireGitlabCredentials(ctx);
    const runs = await GitlabAPI.getWorkflowRuns(credentials, dataSource.owner, dataSource.repo, {
    ...
    if (!workflowRuns.length)
      return `EMPTY: ${dataSource.owner}/${dataSource.repo} has no pipeline runs.`;
    return `GitLab Pipeline Runs (${dataSource.owner}/${dataSource.repo}) - ${workflowRuns.length}:\n\n${workflowRuns
      .map(
        (run, index) =>
          `${index + 1}. ${run.name}: ${run.status}${run.conclusion ? ` / ${run.conclusion}` : ''} [${run.event}]`,
      )
      .join('\n')}`;
  },
  ...
  async gitlab_repo_stats(ctx, dataSource) {
    requireRepo(dataSource.owner, dataSource.repo);
    const credentials = requireGitlabCredentials(ctx);
    const stats = await GitlabAPI.getRepoStats(credentials, dataSource.owner, dataSource.repo);
    return [
