...
export function register(channelEngine, featureStorage) {
  ...
  ipcMain.handle('get-channels', () => {
    try {
      return { ok: true, channels: channelEngine.getAll() };
    } catch (err) {
  ...
  ipcMain.handle('remove-channel', (_e, name) => {
    try {
      channelEngine.removeChannel(name);
      return { ok: true };
    } catch (err) {
      return { ok: false, error: err.message };
    }
  ...
  ipcMain.handle('toggle-channel', (_e, name, enabled) => {
    try {
      channelEngine.toggleChannel(name, enabled);
      return { ok: true };
    } catch (err) {
      return { ok: false, error: err.message };
    }
  ...
  ipcMain.handle('validate-channel', async (_e, name, credentials) => {
    try {
      ...
      if (name === 'whatsapp') {
        const info = await channelEngine.validateWhatsApp(
  ...
  ipcMain.handle('channel-reply', (_e, id, text) => {
    try {
      channelEngine.resolveReply(id, text);
      return { ok: true };
    } catch (err) {
      return { ok: false, error: err.message };
    }
