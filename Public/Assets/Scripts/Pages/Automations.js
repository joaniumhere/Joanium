...
function appendFolderSubs(fieldsEl, data) {
  ...
  fieldsEl.appendChild(makeToggleRow({ checkClass: 'sub-open-terminal', checked: data.openTerminal ?? false, icon: '💻', labelText: 'Open terminal here', subEl: cmdWrap }));
}
...
function appendNotifSubs(fieldsEl, data) {
  ...
  fieldsEl.appendChild(makeToggleRow({ checkClass: 'sub-click-url', checked: !!(data.clickUrl), icon: '🔗', labelText: 'Open URL when notification is clicked', subEl: urlWrap }));
}
...
function appendGithubCheckSubs(fieldsEl, type, data) {
  ...
  fieldsEl.appendChild(makeToggleRow({ checkClass: 'sub-filter-state', checked: !!(data.state && data.state !== 'open'), icon: '🔍', labelText: 'Filter by state (default: open)', subEl: stateWrap }));
}
...
function collectActionFromRow(row) {
  ...
  switch (type) {
    ...
    case 'run_command':
      ...
      action.notifyOnFinish = getCb('sub-notify-finish'); break;
    case 'run_script':
    ...
    case 'write_file':
      ...
      action.append = getCb('sub-append'); break;
    case 'move_file':
    case 'copy_file':
