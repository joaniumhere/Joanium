...
const REFRESH_BUTTON_HTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
...
const CLEAR_BUTTON_HTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
...
function ensureUsageStyles() {
  ...
  style.textContent = `
    ...
    .dow-bar-fill {
      height: 100%;
      border-radius: 999px;
      background: linear-gradient(90deg, var(--accent), color-mix(in srgb, var(--accent) 60%, #7c5dff));
  ...
  <div class="usage-scroll">
    <div class="usage-page-header">
      ...
      <div class="usage-header-actions">
        ...
        <button class="usage-refresh-btn" id="refresh-btn">${REFRESH_BUTTON_HTML}</button>
        <button class="usage-clear-btn" id="clear-usage-btn">${CLEAR_BUTTON_HTML}</button>
      </div>
    </div>
...
function computeStats(records) {
  ...
  for (const record of records) {
    ...
    if (!byHour[hour]) {
      byHour[hour] = { calls: 0, tokens: 0 };
    }
    byHour[hour].calls += 1;
    byHour[hour].tokens += input + output;

    if (!byDow[dow]) {
      byDow[dow] = { calls: 0, tokens: 0, cost: 0 };
    }
    byDow[dow].calls += 1;
...
function renderInsights(stats, records) {
  ...
  if (totalTokens > 0) {
    ...
    const verdict = outputPercent > 60
  ...
  const efficientModels = Object.entries(stats.byModel)
    .filter(([, value]) => value.cost > 0 && (value.input + value.output) > 0);
  ...
  if (costLeader) {
    ...
    insights.push({
      ...
      text: `<strong>${providerLabel(providerId)}</strong> accounts for <strong>${shareOfTotal}%</strong> of your total spend (${fmtCost(providerData.cost)}) across ${providerData.calls} call${providerData.calls !== 1 ? 's' : ''}.`,
    });
  }

  if (stats.count > 0 && stats.totalOutput > 0) {
    const avgOutput = Math.round(stats.totalOutput / stats.count);
    const verbosity = avgOutput > 800
...
function renderChart(byDay, range) {
  ...
  if (titleElement) {
    titleElement.textContent = range === 'all'
      ? 'Last 30 days (tokens)'
      : range === 'today'
        ? 'Today (tokens)'
  ...
  days.forEach((date, index) => {
    ...
    if (index % step === 0 || index === days.length - 1) {
      ...
      labelsHTML += `<text x="${x + barWidth / 2}" y="${height - 8}" text-anchor="middle"
  ...
  yTicks.forEach(tick => {
    ...
    yLabels += `
      <text x="${paddingLeft - 4}" y="${yPos + 3}" text-anchor="end" font-size="8"
        fill="var(--text-muted)" font-family="var(--font-mono)">${fmtTokens(Math.round(maxValue * tick))}</text>
      <line x1="${paddingLeft}" y1="${yPos}" x2="${width - paddingRight}" y2="${yPos}"
    ...
    return `
      <div class="dow-row">
        ...
        <div class="dow-bar-track">
          <div class="dow-bar-fill" style="width:${percent}%"></div>
        </div>
...
function renderCostTable(byModel) {
  ...
  if (!rows.length) {
    element.innerHTML = '<div class="cost-empty">No cost data for this period.</div>';
    return;
  }
...
function renderModelRows(byModel) {
  ...
  if (!rows.length) {
    element.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:8px 0">No model data yet</div>';
    return;
  }
...
function renderProviders(byProvider) {
  ...
  if (!rows.length) {
    element.innerHTML = '<div style="color:var(--text-muted);font-size:13px;grid-column:1/-1">No provider data yet</div>';
    return;
  }
...
function renderActivity(records) {
  ...
  if (!recent.length) {
    element.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:18px;text-align:center">No activity yet - start chatting!</div>';
    return;
  }

  element.innerHTML = recent.map(record => {
    const total = (record.inputTokens ?? 0) + (record.outputTokens ?? 0);
    const cost = tokenCost(record.model, record.inputTokens ?? 0, record.outputTokens ?? 0);
    return `
