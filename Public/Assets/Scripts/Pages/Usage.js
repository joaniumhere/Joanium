...
let _records  = [];
let _range    = 'today'; // 'today' | '7' | '30' | 'all'
let _pollTimer = null;
...
function renderChart(byDay, range) {
  ...
  days.forEach((date, i) => {
    ...
    // label every Nth day to avoid clutter
    const step = days.length <= 7 ? 1 : days.length <= 14 ? 2 : 5;
    if (i % step === 0 || i === days.length - 1) {
      const label = days.length === 1 ? 'Today' : date.slice(5); // MM-DD
...
function renderSummary(stats, records) {
  ...
  const avgPerMsg = stats.count > 0
    ? Math.round((stats.totalInput + stats.totalOutput) / stats.count)
    : 0;
  ...
  const cards = [
    ...
    },
    {
      icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
               ...
               <path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6" stroke-linecap="round"/>
             </svg>`,
      label: 'Est. cost',
    ...
    {
      icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
               <path d="M4 4h16v4H4zM4 12h16v4H4z" stroke-linejoin="round"/>
             </svg>`,
      label: 'Input tokens',
...
function showEmpty() {
  const main = document.querySelector('.usage-scroll');
  ...
  const sections = ['chart-section','provider-section','model-section','activity-section'];
  sections.forEach(id => {
...
function startPolling() {
  ...
  _pollTimer = setInterval(load, 5000); // refresh every 5s
}
